...
vector<long double> external_root(Poly& pol,Poly& diff,long double extreme_p, long double limit_sign,long double init_step, function<int(long double)>& value_sign, long double epsilon) {
    long double value_ex, sign_ex, value_x, sign_x, a, b;
    ...
    if (fabs(value_ex) <= epsilon) {
        ...
        if (sign_ex != limit_sign) {
            ...
            sols_poly.emplace_back(bisection_n_newtoon(pol, diff, a, b,value_sign,epsilon));
        }
    }
    return sols_poly;
}

vector<long double> internal_roots(Poly& pol, Poly& diff, vector<long double>& diff_sols, function<int(long double)>& value_sign, long double epsilon) {
    long double a, b, val_a, val_b, sign_a, sign_b;
    vector<long double> sols_poly;
...
vector<long double> Solve(Poly Pol_solve,bool show_progress, long double epsilon) {
    ...
    function<long double(vector<long double>)> find_max_fabs = [](vector<long double> m) {
        return *max_element(m.begin(), m.end(), [](long double& a, long double& b){
                return fabs(a) < fabs(b); });
    };
    function<long double(vector<long double>)> find_max = [](vector<long double> m) {
        return *max_element(m.begin(), m.end());
    };

    function<int(long double)> value_sign = [](long double x) {
        if (x > 0) {
            return 1;
        }
        return -1;
    ...
    while (degree > 1) {
        ...
        derv.emplace_back(dervcoff);
        /*if (degree == 986) {
            for (long double mem : dervcoff)
                cout << mem << ", ";
            cout << endl;
            Poly a(dervcoff);
            cout << a.eval(2.651696055589838) << endl;
        }*/
        if(show_progress)
    ...
    for (int i = stack_size - 1; i > 0; i--) {
        ...
        long double step = epsilon;
   
  
        //left external root
        external_left = external_root(derv[i - 1], derv[i], left_ex, left_limit_sign, -step, value_sign, epsilon);
        sols_poly.insert(sols_poly.end(), external_left.begin(), external_left.end());
        vector<long double>().swap(external_left);

        //internal roots
        internal = internal_roots(derv[i - 1], derv[i], sols_derv, value_sign, epsilon);
        sols_poly.insert(sols_poly.end(), internal.begin(), internal.end());
        vector<long double>().swap(internal);
        internal.reserve(10);

        //right external root
        external_right = external_root(derv[i - 1], derv[i], right_ex, right_limit_sign, step, value_sign, epsilon);
        sols_poly.insert(sols_poly.end(), external_right.begin(), external_right.end());
        vector<long double>().swap(external_right);
...
long double bisection_n_newtoon(Poly& pol, Poly& derv, long double &a, long double &b, function<int(long double)>& value_sign, long double epsilon) {
    int i;
    ...
    while (fabs(fm) > epsilon && iter < 100) {
        ...
        if (value_sign(fa) != value_sign(fm)) {
            b = m;
            fb = fm;
        }
        if (value_sign(fb) != value_sign(fm)) {
            a = m;
            fa = fm;
        }

        if (fabs(a- old_a) < epsilon && fabs(b - old_b) < epsilon) {
            break;
        }
