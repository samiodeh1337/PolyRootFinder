...
vector<long double> external_root(Poly& pol,Poly& diff,long double extreme_p, long double limit_sign,long double init_step) {
...
vector<long double> Solve(Poly Pol_solve,bool show_progress) {
    ...
    vector<long double> thread_sols_mid;
    ...
    for (int i = stack_size - 1; i > 0; i--) {
        ...
        thread thread_1(internal_roots, derv[i - 1], derv[i], sols_derv, std::ref(thread_sols_mid));
        thread_1.join();
